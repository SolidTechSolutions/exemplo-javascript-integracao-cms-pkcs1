# 🇧🇷 SolidSign API - Exemplo de Integração: Assinatura CAdES (CMS) com PKCS#1 (extensão de navegador, dois passos) (JavaScript)

## Requisitos

- Express / Node.js (`npm install`)
- Um token JWT válido (`POST /solidsign/auth/token`)
- Front-end de referência (opcional): [`exemplo-react-cms-pkcs1`](https://github.com/SolidTechSolutions/exemplo-react-cms-pkcs1)
...
O serviço sobe em `http://localhost:8092`.
...
- `POST /api/cms/pkcs1/prepare/form`
- `POST /api/cms/pkcs1/finalize/form`
...
# 🇬🇧 SolidSign API - Integration Example: CAdES (CMS) Signing with PKCS#1 (browser extension, two-step) (JavaScript)

## Requirements

- Express / Node.js (`npm install`)
- A valid JWT token (`POST /solidsign/auth/token`)
- Reference front-end (optional): [`exemplo-react-cms-pkcs1`](https://github.com/SolidTechSolutions/exemplo-react-cms-pkcs1)
...
The service starts on `http://localhost:8092`.
